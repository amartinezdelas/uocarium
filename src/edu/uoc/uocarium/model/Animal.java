...
public abstract class Animal extends Item implements Movable{
	...
	private void setStatus(AnimalStatus status) {
		this.status = status;//asigna primero el status
		//después borra el "sufijo" de la imagen para reescribirlo
		setSpriteImage(getSpriteImage().replaceAll("_healthy.png", ""));
		setSpriteImage(getSpriteImage().replaceAll("_sick.png", ""));
		setSpriteImage(getSpriteImage().replaceAll("_dead.png", ""));
		if(status==AnimalStatus.DEAD) {//si status es dead, añade String dead
			setSpriteImage(getSpriteImage()+"_dead.png");
		}
		else if(status==AnimalStatus.SICK){//si status es sick, añade String sick
			//según instrucciones del foro, duplico la imagen _healthy.png y la nombro como _sick.png
			setSpriteImage(getSpriteImage()+"_sick.png");
		}	
		else { //si status no es dead ni sick, añade String healthy
			setSpriteImage(getSpriteImage()+"_healthy.png");
		}
	}
